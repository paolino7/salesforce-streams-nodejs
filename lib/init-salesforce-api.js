...
function* initSalesforceApi(env, forceComVersion, logger = loggers.default) {
  logger(`[DEBUG] Identity SFDC`);
  if (env.SALESFORCE_URL != null) {
    logger(`[DEBUG] Connect OAUTH: ${env.SALESFORCE_URL}`);
    // Use OAuth client
    ...
    logger(`[DEBUG] Connect AccessToken: ${env.SALESFORCE_INSTANCE_URL}`);
    ...
    logger(`[DEBUG] Connect Username/Password: ${env.SALESFORCE_LOGIN_URL}`);
    ...
    const identity2 = yield Episode7.call(
      [salesforceApi, salesforceApi.login],
      env.SALESFORCE_USERNAME,
      env.SALESFORCE_PASSWORD
    );
    
    logger(`[DEBUG] identity2: ${JSON.stringify(identity2)}`);
